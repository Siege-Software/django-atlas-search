...
class TestAtlasSearchSignals(TestCase):
    def setUp(self):
        self.genre = GenreFactory()
        self.artist = ArtistFactory()
        self.song = SongFactory(genre=self.genre)
        self.search_index_class = self.song.search_index_class
        self.database_name = self.search_index_class.database_name
        self.collection_name = self.search_index_class.collection_name

    def test_post_save_atlas_search_models(self):
        song_document = get_document(self.database_name, self.collection_name, self.song.pk)
        ...
        # Release date is stored as datetime in MongoDB, not timestamp
        self.assertIsNotNone(song_document.get("release_date"))
    ...
    def test_pre_delete_atlas_search_models(self):
        song_pk = self.song.pk

        song_document = get_document(self.database_name, self.collection_name, song_pk)
        self.assertIsNotNone(song_document)
    ...
    def test_m2m_changed_atlas_search_models(self):
        song_document = get_document(self.database_name, self.collection_name, self.song.pk)
        self.assertIsNotNone(song_document)
